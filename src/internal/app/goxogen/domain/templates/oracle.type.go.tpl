...
func ({{ $short }} *{{ .Name }}) Insert(db pgxdb.Connect) error {
	var err error
	...
	db.WriteLog(sqlstr, time.Since(start), {{ fieldnames .Fields $short .PrimaryKey.Name }}, nil)
...
{{ if ne (fieldnames .Fields $short .PrimaryKey.Name) "" }}
	...
	func ({{ $short }} *{{ .Name }}) Update(db pgxdb.Connect) error {
		var err error
	...
	func ({{ $short }} *{{ .Name }}) Save(db pgxdb.Connect) error {
		if {{ $short }}.Exists() {
...
func ({{ $short }} *{{ .Name }}) Delete(db pgxdb.Connect) error {
	var err error
