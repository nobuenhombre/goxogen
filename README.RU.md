...
|------------|--------|----------|
| **goxogen** | v0.16.0 | XO-пайплайн генерации кода — создаёт Go-модели и функции запросов из схемы PostgreSQL через 9-шаговый автоматизированный процесс |
...
- **Пайплайн XO-генерации** — 9-шаговый процесс: схема БД → модели → функции запросов (one/many/UID) → извлечение репозиториев → агрегатный репозиторий → Wire provider → форматирование
...
goxogen запускает 9-шаговый пайплайн при выполнении с `-config=config.yaml`. Он подключается к PostgreSQL, генерирует модели и функции запросов, извлекает интерфейсы репозиториев и производит чистый отформатированный Go-код.
...
|| 7 | **generateDbRepo** | Сканирует `*-repo.xo.go`, генерирует `a-db-repo.go` через встроенный шаблон `a-db-repo.go.tpl` (`text/template`) с агрегатной структурой `Db{DbName}Repo` + конструктором `NewDb{DbName}Repository` (создаёт подключение к БД внутри через `pgxdb.NewDB`) + метод `Close()` |
| 8 | **generateProvider** | Генерирует `provider.go` с Wire `ProviderSet`, экспортирует `Provider{DbName}` с cleanup |
| 9 | **goFormatCode** | Запускает `go fmt`, `goimports -w`, `go vet` для чистого вывода |
...
15 Go-шаблонов встроены в бинарник goxogen через `//go:embed templates/*.tpl`. 14 XO-шаблонов извлекаются во временную директорию при запуске для xo CLI. Шаблон `a-db-repo.go.tpl` загружается напрямую из встроенной FS через `text/template` для шага 7.
...
|| `a-db-repo.go.tpl` | Шаблон агрегатной структуры DbRepo (используется в generateDbRepo) |
